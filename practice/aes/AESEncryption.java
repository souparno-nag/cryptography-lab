public class AESEncryption {
    private final int Nk, Nr;
    private final byte[] roundKey;

    private static final int[] SBOX = {
        
    };

    private static final int[] INV_SBOX = {
        
    };

    private static final byte[] RCON = {
        0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,(byte)0x80,0x1B,0x36
    };

    public AESEncryption(byte[] key, int mode) {
        if (mode == 1 && key.length == 16) {Nk = 4; Nr = 10;}
        else if (mode == 2 && key.length == 24) {Nk = 6; Nr = 12;}
        else if (mode == 3 && key.length == 32) {Nk = 8; Nr = 14;}
        else throw new IllegalArgumentException("Key must be 1, 24 or 32 bytes");
        roundKey = keyExpansion(key);
    }

    static byte mul2(byte b) {
        int x = b & 0xFF;
        x = (x << 1) ^ ((x & 0x80) != 0 ? 0x1B : 0);
        return (byte) x;
    }
    static byte mul3(byte b) {
        return (byte) (mul2(b) ^ b);
    }
    static byte mul9(byte b) {
        return (byte) (mul2(mul2(mul2(b))) ^ b);
    }
    static byte mul11(byte b) {
        return (byte) (mul2(mul2(mul2(b))) ^ mul2(b) ^ b);
    }
    static byte mul13(byte b) {
        return (byte) (mul2(mul2(mul2(b))) ^ mul2(mul2(b)) ^ b);
    }
    static byte mul14(byte b) {
        return (byte) (mul2(mul2(mul2(b))) ^ mul2(mul2(b)) ^ mul2(b));
    }

    static byte[] rotL(byte[] a, int n) {
        byte[] r = new byte[4];
        for (int i = 0; i < 4; i++) r[i] = a[(i + n)%4];
        return r;
    }

    static byte[] rotR(byte[] a, int n) {
        byte[] r = new byte[4];
        for (int i = 0; i < 4; i++) r[(i + n)%4] = a[i];
        return r;
    }

    byte[][] toState(byte[] in) {
        byte[][] s = new byte[4][4];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                s[i][j] = in[4*i+j];
            }
        }
        return s;
    }

    byte[] fromState(byte[][] s) {
        byte[] out = new byte[16];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                out[4*i+j] = s[i][j];
            }
        }
        return out;
    }

    void subBytes(byte[][] s) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                s[i][j] = (byte) SBOX[s[i][j] & 0xFF];
            }
        }
    }

    void InvSubBytes(byte[][] s) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                s[i][j] = (byte) INV_SBOX[s[i][j] & 0xFF];
            }
        }
    }

    void shiftRows(byte[][] s) {
        for (int i = 1; i < 4; i++) s[i] = rotL(s[i], i);
    }

    void invShiftRows(byte[][] s) {
        for (int i = 1; i < 4; i++) s[i] = rotR(s[i], i);
    }

    void mixColumns(byte[][] s) {
        for (int c = 0; c < 4; c++) {
            byte a0 = s[0][c], a1 = s[1][c], a2 = s[2][c], a3 = s[3][c];
            s[0][c] = (byte) (mul2(a0) ^ mul3(a1) ^ a2 ^ a3);
            s[1][c] = (byte) (a0 ^ mul2(a1) ^ mul3(a2) ^ a3);
            s[2][c] = (byte) (a0 ^ a1 ^ mul2(a2) ^ mul3(a3));
            s[3][c] = (byte) (mul3(a0) ^ a1 ^ a2 ^ mul2(a3));
        }
    }

    void invMixColumns(byte[][] s) {
        for (int c = 0; c < 4; c++) {
            byte a0 = s[0][c], a1 = s[1][c], a2 = s[2][c], a3 = s[3][c];
            s[0][c] = (byte) (mul14(a0) ^ mul11(a1) ^ mul13(a2) ^ mul9(a3));
            s[1][c] = (byte) (mul9(a0) ^ mul14(a1) ^ mul11(a2) ^ mul13(a3));
            s[2][c] = (byte) (mul13(a0) ^ mul9(a1) ^ mul14(a2) ^ mul11(a3));
            s[3][c] = (byte) (mul11(a0) ^ mul13(a1) ^ mul9(a2) ^ mul14(a3));
        }
    }

    void addRoundKey(byte[][] s, int r) {
        int off = r * 16;
        for (int c = 0; c < 4; c++) {
            for (int i = 0; i < 4; i++) {
                s[i][c] ^= roundKey[off + 4*c + i];
            }
        }
    }

    byte[] rotWords(byte[] w) {return new byte[]{w[1], w[2], w[3], w[0]};}

    byte[] subWords(byte[] w) {
        for (int i = 0; i < 4; i++) w[i] = (byte) SBOX[w[i] & 0xFF];
        return w;
    }

    byte[] keyExpansion (byte[] key) {
        int words = 4 * (Nr + 1);
        byte[] w = new byte[4 * words];
        System.arraycopy(key, 0, w, 0, key.length);
        byte[] temp = new byte[4];
        int i = Nk;
        while (i < words) {
            for (int j = 0; j < 4; j++) temp[i] = w[4 * (i - 1) + j];
            if (i % Nk == 0) {
                temp = subWords(rotWords(temp));
                temp[0] ^= RCON[i/Nk];
            } else if (Nk > 6 && i % Nk == 4) temp = subWords(temp);
            for (int j = 0; j < 4; j++) w[4* i + j] = (byte) (w[4 * (i - Nk) + j] ^ temp[j]);
        }
        return w;
    }

    public byte[] encryptBlock(byte[] in) {
        byte[][] s = toState(in);
        addRoundKey(s, 0);
        for (int r = 1; r < Nr; r++) {
            subBytes(s);
            shiftRows(s);
            mixColumns(s);
            addRoundKey(s, r);
        }
        subBytes(s);
        shiftRows(s);
        addRoundKey(s, Nr);
        return fromState(s);
    }

    public byte[] decryptBlock(byte[] in) {
        byte[][] s = toState(in);
        addRoundKey(s, Nr);
        for (int r = Nr - 1; r  > 0; r--) {
            invShiftRows(s);
            invShiftRows(s);
            addRoundKey(s, r);
            mixColumns(s);
        }
        InvSubBytes(s);
        invShiftRows(s);
        addRoundKey(s, 0);
        return fromState(s);
    }
}